...
const SortableItem = SortableElement(({ value }) => <li className='excel-item excel-drag-item'>{value}</li>)
...
export class YakaEditTable extends Component {
	...
	createDataSource = props => {
		...
		if (value !== null) {
			const values = value.filter(val => val !== null).map(val => {
				val.key = uuidv4()
				return val
			})
			this.setState({
	...
	render() {
		...
		return (
			<div className='yaka-edit-table'>
				...
				/>
				<Modal title='匹配 Excel 字段' visible={visible} onOk={this.handleOk} onCancel={this.handleCancel}>
